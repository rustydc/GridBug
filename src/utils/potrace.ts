...
var Potrace = (function() {
    ...
    function setParameter(obj: Partial<IInfo>): void {
     var key;
     for (key in obj) {
    ...
    function bmToPathlist(): void {
      ...
      function findNext(point: Point): Point | boolean {
    ...
    function processPath(): void {
    
      function Quad() {
        this.data = [0,0,0,0,0,0,0,0,0];
      }
  
      Quad.prototype.at = function(x: number, y: number): number {
        return this.data[x * 3 + y];
      };
      
      function Sum(x: number, y: number, xy: number, x2: number, y2: number) {
        this.x = x;
        this.y = y;
        this.xy = xy;
        this.x2 = x2;
      ...
      function quadform(Q: any, w: Point): number {
      ...
      function optiCurve(path: Path): void {
        ...
        function opti_penalty(path: Path, i: number, j: number, res: any, opttolerance: number, convc: number[], areac: number[]): number {
    ...
    function getSVG(size: number, opt_type?: string): string {
    
      function path(curve: ICurve): string {
      
        function bezier(i: number): string {
